...
**Source of this inspection** identifies the file name/size or the browser copy identifier, revision and saved timestamp actually read. A different selection or later autosave does not silently refresh the opened contents; inspect again to read that copy. These labels describe the local source, not ledger authority.

...
Two initial targeted runs had **22 passed / 6 failed** and **26 passed / 6 failed**. The new wait guard had changed the precedence of six existing error messages. Restoring the previous synchronous guard order retained the new lifecycle checks. The final targeted run passed **33 tests / 5 files in 91.48 seconds**, including delayed SDK work before checkpoint and delayed journal storage completion after timeout. Neither late continuation invoked broadcast; the committed-write case retained the encrypted identifier. **18 Chrome desktop/mobile E2E checks** passed in **1.3 minutes** across the existing role journal/file/device-copy, IndexedDB concurrency/removal and workspace-switching flows. Final normal web build/typecheck and release validation passed: **8 circuits / 80 files / 65,457,959 bytes**. `git diff --check` passed.

[Evidence](evidence/role-preparation-deadline.json) retains all attempts and scope. The preparation timer covers the wrapped SDK call, not earlier connection/input construction or later reads. This does not cancel already invoked native wallet work, authenticate finality or supply the combined demo's missing durable identifier journal. Native-wallet validation, full regression and runtime image refresh remain separate work.

## Full web regression and refreshed autosave runtime -- 2026-09-11

At application revision `0ada0b2`, the complete web unit/component suite passed **374 tests / 59 files in 189.22 seconds**, with two workers. The full ordinary desktop/mobile E2E run finished with **136 passed / 2 failed in 6.3 minutes**, no automatic retries. Both failures were the stalled-upload recovery test's obsolete v4 assertion; the application now exports v5. The corrected test checks v5 and its default program draft while retaining pending/uncertainty checks, isolated-context restoration and identical ciphertext upload/readback. Both corrected desktop/mobile cases passed in **53.6 seconds**. Coverage is **138 E2E cases across these runs**, not one successful full invocation. [Regression evidence](evidence/web-autosave-regression.json) preserves both attempts and the assertion change.

The final normal web build/typecheck and artifact check passed with **8 circuits / 80 files / 65,454,274 bytes**. The new local runtime image is `sha256:ee5772a8bba9fa96ff820de55505f91ad75a87b198a1b104ac0631a585eb8859`. Docker validated the copied release and Caddy configuration; Go compilation/matcher-test layers were cached. Its image-pinned container drill passed at **2026-09-11T03:31:47.862Z**, verifying every hosted file hash, headers/404s, non-root/read-only execution, desktop/mobile public/deployment workers, timeout/retry/cancel, cross-origin widget isolation, exchange navigation/clearing and graceful restart. Cleanup completed before success publication.

The exact-image scan completed with exit **1**: **0 OS findings** across **32 Alpine 3.23.5 packages**, and **1 unsuppressed UNKNOWN GO-2026-5932** finding in Caddy's x/crypto v0.56.0. The strict security gate remains failed. Final web-test/scanner ownership-label queries were empty. [Runtime evidence](evidence/web-autosave-runtime.json) and [current container drill](evidence/web-container-drill.json) retain identities and raw-log hashes. Other workspaces, native-wallet behavior, fresh proving-key generation and public deployment were not rerun or claimed.
...
In this successful SQLite run, the first upload took **808 ms** to response headers: the adapter `put` took **796 ms**, with **795 ms** inside `COMMIT`. A readiness operation took **1001 ms**, including a **999 ms** commit. These measurements locate the observed delay primarily within SQLite commit, rather than envelope parsing or transport. They do not identify individual filesystem synchronization calls or retroactively establish each earlier failed request's timing.

A separate restricted-container experiment kept `synchronous=FULL` and compared DELETE, PERSIST and TRUNCATE journals in forward/reverse order, five synthetic 512-byte row commits per round. Reopen row counts and integrity checks passed. Across ten commits per mode, median/max were **586/1126 ms**, **954/1192 ms**, and **453.5/1017 ms**, respectively. Every mode exceeded one second at least once; these small uncontrolled samples do not justify changing the runtime journal mode or relaxing the drill timeout. SQLite documents journal-mode alternatives and commit synchronization costs in its [PRAGMA reference](https://www.sqlite.org/pragma.html#pragma_journal_mode) and [atomic commit description](https://www.sqlite.org/atomiccommit.html).

The experiment removed its fixture and exited 0. The first volume cleanup could not parse the Docker Go template through the Windows wrapper; full JSON inspection then verified the exact owner label and explicit cleanup succeeded. Final diagnostic/drill container and volume queries were empty. [Raw trace records, timing samples, log/script hashes and limits](evidence/cipherstore-commit-latency.json) retain the evidence. The runtime image, journal configuration and timeout remain unchanged; this traced success does not clear the prior concurrent lost-acknowledgment failures or establish a production SLO.
...
The API build passed. **34 web tests / 4 files passed in 13.40 seconds**, covering widget behavior, invitation links, join review and role recovery. The final widget-only run passed all **5 tests**, including anonymous/no-referrer snippet attributes. Chrome desktop and Pixel 7 passed **8 E2E cases in 51.0 seconds**, two workers and no retries: an actual second HTTP server hosts the cross-origin embed; only the two public modules load, there are no API POSTs, long identifiers fit the mobile viewport, and the opened tab reviews the intended program with `window.opener === null`. Invalid attribute replacement removes the link. A separate JavaScript-disabled context retains the fallback link, and existing invitation journeys pass.

Two preceding browser invocations each had **6 passes and 2 failures** (1.2 and 1.1 minutes respectively). Their route-fulfilled publisher pages triggered Chrome's local-network restrictions before the loopback script loaded: first an insecure/public-origin-to-loopback rejection, then denied loopback permission for the synthetic response. Replacing the publisher fixture with a real loopback HTTP server on a distinct port resolved the test setup; no browser security feature was disabled and no application assertion was relaxed. This proves local cross-origin behavior, not public TLS hosting.

The Caddy configuration enables wildcard CORS only for the two public module path patterns. `caddy validate` exited **0** using existing image `sha256:6666fdd2a7c4a815d4a8344199eb0ec257bcec89bd104faf8235e8fa492aa81c` with the new config mounted. A separate loopback container served the current build/config through that Caddy binary; both module responses had the required CORS header, the homepage did not, and the uniquely labelled container was removed. This is a mounted-artifact host check, not a rebuilt distributable image or a new security scan.

The final normal web build with TypeScript checking and package gate exited **0**: **8 circuits, 73 files, 64,103,964 bytes**. The gate now explicitly rejects a missing widget entrypoint; all **8 release-tool tests passed**, and the final read-only package check also exited **0**. Logs: `.compact/widget-tests.log`, `.compact/widget-tests-final.log`, `.compact/widget-browser-real-host.log`, `.compact/widget-host-drill.log`, `.compact/widget-release.log` and `.compact/widget-release-tests.log`. Native Lace completion through the widget, refreshed image/public hosting, published SDK and authenticated timeline remain open. [Embedding instructions and deployment limits](submission-widget.md).
...
`scan-container.sh` now bounds image inspection, export and the scanner client with coreutils-compatible `timeout -k`, preserving Trivy's original all-severity failure gate and ten-minute scan limit. New scans carry a unique name/ownership label. Cleanup uses bounded queries, refuses foreign ownership, avoids repeating an in-progress removal, preserves exports when cleanup is uncertain and converts an otherwise successful scan into failure if cleanup fails. Original nonzero statuses are preserved.

Eight POSIX harness cases passed under WSL Node **22.14.0** in **0.490 seconds**, exit **0**, using synthetic Docker/timeout executables: clean completion, findings, client timeout with removal pending, foreign ownership, failed removal, stopped/active cleanup after timeout, failed container listing and failed export. They execute the real shell script and verify cleanup/status behavior and deadline arguments without creating real containers or rerunning Trivy. CI's Linux validation job now includes this test; Windows-native runs skip it. This script-only harness does not change the application's Node requirement.

The initial harness had four cleanup failures because Node `rmSync` without recursion cannot remove an empty directory. Replacing that call with `rmdirSync` corrected it; the four identified synthetic fixtures were removed after path checks. After the harness run, authoritative Docker checks showed both old containers absent and the original private scan directory removed. The existing scanner wrapper exited 1 with its finding intact. This later cleanup does not turn the earlier timed-out web drill into a pass, and is not attributed to the script changes. [Scanner operational limits](go-vulnerability-checks.md).

After Docker finished the earlier removals, the updated web-container drill was rerun against the same image and exited **0**. At **2026-09-10T09:46:38.251Z**, all 69 asset checks, security headers, non-root/read-only configuration, desktop/mobile public lookup and packaged deployment-worker cases, and graceful restart passed. The result was published only after cleanup; a subsequent label-filtered Docker query found no remaining web-test container. [Current drill result](evidence/web-container-drill.json) and [initial/final run distinction](evidence/web-worker-runtime.json) preserve the earlier failure. The scanner finding remains unresolved.

## Refresh local web image and exercise packaged workers -- 2026-09-10

The local web image now contains application revision `03f34fe`, including role recovery changes and the deployment-policy worker: **8 circuits, 69 files, 64,010,347 artifact bytes**. The current image is `sha256:6666fdd2a7c4a815d4a8344199eb0ec257bcec89bd104faf8235e8fa492aa81c`, config `sha256:b82643dfcbee6b55b1e9bd183134c9177648be67ff066371713c2f0afe8c1c8e`. Both initial export invocations reported exit **1** through PowerShell/WSL despite complete Docker export output and an inspectable image. A separate cached build with `type=cacheonly`, capturing the direct child status via Python, exited **0** without changing the image tag. These are separate observations, not a claim that the first two invocations passed. The image build gate checked the manifest, module verification and the guarded CEL patch; Caddy's selected upstream tests passed in **0.209 seconds**.

The disposable container's functional checks completed: HTTP artifact hashes, non-root UID, read-only root, headers/CSP, missing-file 404s, desktop/mobile captured-state lookup, packaged worker execution for both matching and mismatched policy, and graceful stop/restart. However, the complete drill exited **1** when owned-container removal exceeded its 60-second timeout. Docker subsequently reported that stopped container as `removing`. The prior helper published success before cleanup; it now publishes only after cleanup succeeds. The retained result is explicitly marked `cleanupCompleted: false` and `drillExitCode: 1`, and the changed helper passed `node --check`.

The exact image's strict Trivy run reports **one UNKNOWN finding, GO-2026-5932 in golang.org/x/crypto v0.56.0**, no fixed version listed, and zero Alpine findings across 32 packages on Alpine 3.23.5. Docker reports the scanner process exited **1**, but its wrapper remains alive waiting for auto-removal. No clean security gate or completed cleanup is claimed. Both removals are already in progress; no duplicate removal or shared Docker/WSL restart was attempted.

[Runtime evidence](evidence/web-worker-runtime.json), [functional checks and incomplete cleanup](evidence/web-container-drill.json), and [exact-image scan observation](evidence/web-current-runtime-scan.json) retain the distinction. No public deployment, native wallet, fresh proving keys, code audit or complete production-readiness claim is introduced.

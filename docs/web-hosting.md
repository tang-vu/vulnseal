...
Both Caddy Dockerfiles also pin patched Alpine packages: c-ares 1.34.8-r0, curl/libcurl 8.22.0-r0 and libcrypto3/libssl3 3.5.8-r0. These versions must remain available in the Alpine 3.23 repository for a fresh build. The first September 9 scan found 93 Alpine findings and 24 Go findings in the original binary. After the Alpine updates and custom Caddy build, both final images have zero critical/high/medium/low findings and one unknown finding, GO-2026-5932. That advisory concerns x/crypto's OpenPGP packages; none occurs in the 993-package build dependency graph. The raw scanner warning is retained rather than suppressed. See [current remediation evidence](evidence/caddy-go-remediation.json) and [the earlier scan](evidence/web-image-vulnerabilities.json). These checks do not establish production readiness or replace an audit.
...
The latest local image refresh at application revision `6a7ec8b` is recorded in [validated runtime evidence](evidence/web-validated-runtime.json). The image packages the 80 checked files from the consolidated regression, including current upload acknowledgment, recovery snapshot and recipient-key fixes. The drill pins the image ID before launch. The extended container drill passed all functional checks, public-worker timeout/retry/cancel, exchange input retention/clearing across navigation, cross-origin widget behavior and owned cleanup. Its exact-image scanner completed but still returned the unsuppressed UNKNOWN finding GO-2026-5932, so the strict security gate remains failed. This does not establish production readiness. Earlier worker-image build/cleanup failures remain recorded in [historical evidence](evidence/web-worker-runtime.json).

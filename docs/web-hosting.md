...
The latest local worker-image refresh and its limits are recorded in [runtime evidence](evidence/web-worker-runtime.json). The first drill timed out during Docker removal. After Docker completed that removal, the updated drill passed all functional checks and cleanup on the same image. It publishes successful evidence only after owned-container cleanup succeeds. The scanner wrapper also completed cleanup, but its unresolved finding still fails the strict security gate; the functional pass does not establish production readiness.

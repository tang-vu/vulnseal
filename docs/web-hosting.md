...
The latest local image packages application revision `4c6a55b`; see [API/recovery runtime evidence](evidence/api-hardening-runtime.json). Its immutable-image localhost drill passed all 80 file checks, desktop/mobile worker/widget/exchange behavior, non-root/read-only operation, restart and owned cleanup. The exact-image scan still reports one unsuppressed UNKNOWN GO-2026-5932 finding; the strict security gate remains failed. The latest complete six-workspace/ordinary-browser source regression is recorded separately at `655397b` in [744-test / 168-browser evidence](evidence/bytes32-full-regression.json); subsequent targeted API/contract checks are listed in the validation report. No public deployment or production-readiness claim is made.

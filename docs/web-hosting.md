...
The latest local image refresh at application revision `38f19dc` is recorded in [public-worker/runtime evidence](evidence/web-public-worker-runtime.json). The image packages 80 checked files, and the extended container drill passed all functional checks, public-worker timeout/retry/cancel, cross-origin widget behavior and owned cleanup. Its exact-image scanner completed but still returned the unsuppressed UNKNOWN finding GO-2026-5932, so the strict security gate remains failed. This does not establish production readiness. Earlier worker-image build/cleanup failures remain recorded in [historical evidence](evidence/web-worker-runtime.json).

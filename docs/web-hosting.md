...
The latest local worker-image refresh and its limits are recorded in [runtime evidence](evidence/web-worker-runtime.json). Functional checks passed, but the drill failed during Docker removal and cleanup remains unconfirmed; the scanner also waits for auto-removal after reporting its unresolved finding. The drill now publishes successful evidence only after owned-container cleanup succeeds. Neither the image refresh nor a functional pass clears the security or production release gates.

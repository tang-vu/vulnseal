...
The latest local image refresh at application revision `819eb1e` is recorded in [recovery runtime evidence](evidence/recovery-full-runtime.json). It packages 80 checked files including the recent recovery deadlines, preparation checkpoint and offline report reading. The complete web suite passed 509 tests, and all 154 ordinary desktop/mobile E2E cases passed; 8 release-tool tests and the root source/build/release gate also passed. See [source regression evidence](evidence/recovery-full-regression.json). The immutable-image container drill passed hosted hashes, functional/worker/widget/exchange checks, restart and owned cleanup. A freshly updated scanner database still reports one unsuppressed UNKNOWN GO-2026-5932 finding in this exact image, so the strict security gate remains failed. No production-readiness claim is made. [Earlier six-workspace regression](evidence/journal-full-regression.json) and [journal runtime](evidence/journal-full-runtime.json) retain their original source/image scope.

...
The latest local image refresh at application revision `25f593c` is recorded in [widget/runtime evidence](evidence/web-widget-runtime.json). The image packages 73 checked files, and the extended container drill passed all functional checks, cross-origin widget behavior and owned cleanup. Its exact-image scanner completed but still returned the unsuppressed UNKNOWN finding GO-2026-5932, so the strict security gate remains failed. This does not establish production readiness. Earlier worker-image build/cleanup failures remain recorded in [historical evidence](evidence/web-worker-runtime.json).

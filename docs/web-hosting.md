...
The image pins Caddy 2.11.4 and the Node validation stage by digest. Runtime UID/GID is 65532, the filesystem is read-only under Compose, capabilities are dropped and privilege escalation is disabled. The upstream binary is replaced by our pinned Go 1.27.1 build of Caddy 2.11.4; the replacement has no binding capability because port 8080 needs none. Build inputs, the CEL compatibility backport and upstream tests are documented in [the runtime build guide](../infra/caddy/README.md). Compose limits memory to 256 MiB, processes to 64 and container log rotation to three 10 MiB files. Caddy's admin API, automatic HTTPS and config persistence are disabled for this internal HTTP listener. The healthcheck fetches the index page; it does not rehash all keys or test downstream services. Runtime logs are available through Docker; request access logging is not enabled by this configuration.

Both Caddy Dockerfiles also pin patched Alpine packages: c-ares 1.34.8-r0, curl/libcurl 8.22.0-r0 and libcrypto3/libssl3 3.5.8-r0. These versions must remain available in the Alpine 3.23 repository for a fresh build. The first September 9 scan found 93 Alpine findings and 24 Go findings in the original binary. After the Alpine updates and custom Caddy build, both final images have zero critical/high/medium/low findings and one unknown finding, GO-2026-5932. That advisory concerns x/crypto's OpenPGP packages; the later shared-stage diagnostic found none of its seven package paths in the 994-package build dependency graph ([evidence](evidence/caddy-runtime-source.json)). The raw scanner warning is retained rather than suppressed. See [current remediation evidence](evidence/caddy-go-remediation.json) and [the earlier scan](evidence/web-image-vulnerabilities.json). These checks do not establish production readiness or replace an audit.
...
The drill expects the `vulnseal-web:local` image and matching local release/compiler files. Set `VULNSEAL_DOCKER_WSL_DISTRO=Ubuntu` when Node must invoke Docker through WSL. It creates a uniquely labelled container on a random localhost port, verifies the artifact over HTTP, non-root/read-only configuration, headers, missing-file responses, captured-state public lookup in desktop/mobile Chrome, packaged deployment-worker decoding with matching and mismatched saved policy, and graceful restart. The worker is loaded from the image under its actual CSP, and its captured raw transaction/state are checked by the shipped SDK. It also hosts a publisher page on a second local HTTP origin, verifies the two public widget modules and their narrow CORS headers, and opens invitation review without opener access on desktop/mobile. It removes only its own labelled container and attached anonymous volumes. Browser service responses are captured fixtures, external requests are intercepted, and no wallet or live chain is used. `--write-evidence` retains the result in `docs/evidence/web-container-drill.json`.

Ordinary CI skips prover-key generation and therefore cannot build this network-capable image from that artifact; the packaging/HTTP rejection tests remain in CI. The separate manually triggered [Release verification workflow](../.github/workflows/release-verification.yml) generates all proving material from a fresh checkout, validates the six workspaces, builds/checks the release and runs this container drill with Chrome. It has a 30-minute full-compilation limit and a 90-minute job limit. A missing key or failed generation stops the job; there is no skip-ZK fallback. This workflow has not yet run remotely. It does not publish an image, deploy a service, perform a native wallet transaction or establish a vulnerability-free hosting image.

The latest local image refresh at application revision `f8ad400` is recorded in [journal runtime evidence](evidence/journal-full-runtime.json). It packages 80 checked files, including elapsed wallet/proof guards, checked deployment reconnect and required report journals. All six workspace unit suites passed 655 tests (471 in web); the complete ordinary desktop/mobile E2E run passed all 148 cases. All workspace typechecks and the root release build passed; see [full regression evidence](evidence/journal-full-regression.json). The image-pinned container drill passed hosted hashes, functional/worker/widget/exchange checks, restart and owned cleanup. Its exact-image scan still reports one unsuppressed UNKNOWN GO-2026-5932 finding, so the strict security gate remains failed. No production-readiness claim is made. [Earlier deployment runtime](evidence/deployment-full-runtime.json), [autosave runtime](evidence/web-autosave-runtime.json) and [historical worker-image failures](evidence/web-worker-runtime.json) remain available.

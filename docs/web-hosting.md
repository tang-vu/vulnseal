...
The latest local image refresh at application revision `0ada0b2` is recorded in [autosave runtime evidence](evidence/web-autosave-runtime.json). It packages 80 checked files, including recovery v5 drafts, encrypted browser checkpoints and optional autosave. The complete web suite passed 374 tests; 138 desktop/mobile E2E cases were covered across the full run and a corrected two-case rerun, as qualified in [regression evidence](evidence/web-autosave-regression.json). The image-pinned container drill passed hosted hashes, functional/worker/widget/exchange checks, restart and owned cleanup. Its exact-image scan still reports one unsuppressed UNKNOWN GO-2026-5932 finding, so the strict security gate remains failed. No production-readiness claim is made. [Earlier validated runtime](evidence/web-validated-runtime.json) and [historical worker-image failures](evidence/web-worker-runtime.json) remain available.

...
The latest local image refresh at application revision `05f20ef` is recorded in [deployment runtime evidence](evidence/deployment-full-runtime.json). It packages 80 checked files, including required deployment checkpoints and confirmed-address recovery updates. The complete web suite passed 407 tests and the complete ordinary desktop/mobile E2E run passed all 144 cases; see [full regression evidence](evidence/deployment-full-regression.json). The image-pinned container drill passed hosted hashes, functional/worker/widget/exchange checks, restart and owned cleanup. Its exact-image scan still reports one unsuppressed UNKNOWN GO-2026-5932 finding, so the strict security gate remains failed. No production-readiness claim is made. [Earlier autosave runtime](evidence/web-autosave-runtime.json), [earlier validated runtime](evidence/web-validated-runtime.json) and [historical worker-image failures](evidence/web-worker-runtime.json) remain available.

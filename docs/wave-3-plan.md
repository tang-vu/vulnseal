...
- Embeddable submission widget and a small typed SDK for program creation, seal verification, and public timelines.
  ...
  A [public verification package entrypoint](public-verification-sdk.md) now shares the web implementation and supports receipt/state comparison directly from Node, with an offline executable example. The [program policy entrypoint](program-policy-sdk.md) prepares constructor parameters for web/Node with a compiled-constructor offline example and documented deployment composition. [Public invitation links](program-invitation-links.md) and an [embeddable submission entry](submission-widget.md) open the independent submission workspace with explicit program review before connecting Lace. The widget keeps private authoring/signing in that separate workspace. Native-wallet completion through the embedded entry, complete provider/recovery setup and authenticated timelines remain open; the existing workspace packages are not published SDK releases.

...
The response uses [Prometheus text exposition 0.0.4](https://prometheus.io/docs/instrumenting/exposition_formats/), with no-store caching. It exposes `vulnseal_http_responses_total{status_class="1xx"..."5xx"}`, `vulnseal_http_aborted_responses_total`, `vulnseal_http_active_requests` and `vulnseal_active_uploads`. Label values come from five fixed status classes; no paths, digests, program identifiers, origins, IPs or report contents are retained in these metrics. Repeated metrics scrapes do not count themselves. Health/readiness probes and other handled requests do count.
...
**Local evaluation only:** the modified collector currently fails the all-severity vulnerability gate with two UNKNOWN occurrences of `GO-2026-5932`, one per binary. Rebuilding dependencies removed the earlier critical/high/medium findings; see [remediation evidence](evidence/prometheus-remediation.json) and the [historical upstream scan](evidence/prometheus-runtime-scan.json). The recorded build graph excludes the advisory's OpenPGP package, but no finding is suppressed and OS clearance remains unverified. Container restrictions and successful scrape tests do not override this failed release gate.

The optional [monitoring Compose overlay](../infra/cipherstore-monitoring.yml) builds modified Prometheus `3.14.0-vulnseal.1` from checksum-pinned source/UI archives and digest-pinned builder/runtime bases (see the [build notes](../infra/monitoring/README.md)), enables cipherstore metrics explicitly and scrapes the service over the Compose network every 15 seconds. Start it using the same project name as your existing ciphertext store so its volume remains attached:

...
| Public observers can inspect a sealed workflow | Independent browser lookup and public receipt/share links, separate session timeline, independent CLI verification | Captured-state deserialization and desktop/mobile E2E tests; live Preprod lookup | Browser lookup trusts indexer/RPC, does not authenticate deployed code or reconstruct history; Preprod may reset | Real current-state lookup; bounded trust |

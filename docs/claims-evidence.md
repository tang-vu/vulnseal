...
| Public observers can inspect a sealed workflow | Indexer-readable public record, browser session timeline, and independent CLI verification | UI component/E2E tests; independent Preprod indexer/RPC snapshot | Browser viewer is session-scoped and is not an independent refresh; public lookup/shareable receipts remain incomplete; Preprod may reset | Real CLI mechanism; partial browser UX |

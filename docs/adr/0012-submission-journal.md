...
### Wallet continuation deadlines

Browser wallet setup, authorization, balancing and post-checkpoint submission now share an elapsed-time guard. Each phase records wall and monotonic deadlines. It rechecks them before returning a result, when handling a rejection, and at each setup continuation. Reaching either clock expires the phase even if the timer callback has not run. Timely errors keep their original identity; late connector resolve/reject outcomes retain the transaction identifier as unknown.

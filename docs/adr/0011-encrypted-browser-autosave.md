...
Autosave covers the current `RoleVault`: role identity, deployment address and prepared/received reports. Unprepared draft edits, transition notes, receiving keys and complete transaction history are still outside that schema. The feature is scoped to one origin/browser profile and does not synchronize across devices. Password recovery, persistent-storage grants, deletion UI and quota management remain separate work. Existing IndexedDB rows are preserved on validation or storage failure.

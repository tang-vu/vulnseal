...
Autosave covers the current `RoleVault`: role identity, deployment address and prepared/received reports. Payload version 2 also includes pre-submission identifiers as described in [ADR-0012](0012-submission-journal.md). Payload version 3 also carries one incomplete researcher report draft, as described in [ADR-0014](0014-encrypted-role-drafts.md). Transition notes, receiving keys and complete transaction history remain outside that schema. The feature is scoped to one origin/browser profile and does not synchronize across devices. Password recovery, persistent-storage grants and quota management remain separate work. Existing IndexedDB rows are preserved on validation or storage failure.

...
The catalog reveals metadata only. Unlock authenticates/decrypts the role backup, then uses the same program/authority/report checks as file recovery before installing a deployed workspace. Restoring an undeployed vendor identity requires no wallet. Autosave resumes from the exact loaded revision; changes made elsewhere before the next write trigger a conflict. The app makes no cross-tab merge attempt.

Autosave covers the current `RoleVault`: role identity, deployment address and prepared/received reports. Payload version 2 also includes pre-submission identifiers as described in [ADR-0012](0012-submission-journal.md). Unprepared draft edits, transition notes, receiving keys and complete transaction history are still outside that schema. The feature is scoped to one origin/browser profile and does not synchronize across devices. Password recovery, persistent-storage grants and quota management remain separate work. Existing IndexedDB rows are preserved on validation or storage failure.

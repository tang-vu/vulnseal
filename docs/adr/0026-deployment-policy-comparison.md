...
The browser worker additionally compares the eight SDK-exposed verifier keys against the `.verifier` files served from its own release directory. Expected entrypoints are an exhaustive typed set; indexer-supplied names never select URLs. It reports matching/different keys and missing/unexpected entrypoints separately from policy fields. Missing/HTML/oversized local files fail the check; each key download is bounded to 64 KiB and uses the existing overall deadline. The standalone comparison helper can omit a reference base, in which case no verifier comparison is returned; the shipped worker always supplies its release-relative base.

This is equality with the release's reference key bytes, not proof of source-to-key generation or a signed release. The SDK exposes only its latest supported key version, so matching these bytes does not exclude additional versions. Retained proving material still has its existing provenance limits. The current or future contract may also differ from its historical deployment after authorized maintenance.

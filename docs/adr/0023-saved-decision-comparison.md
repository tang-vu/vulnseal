...
| `submitRetest` | Explicit v9 Pass/Fail choice and retest commitment derived from exact saved notes, report ID, replayed patch commitment and saved choice |
...
Retest comparison requires both the immutable note snapshot and explicit v9 choice, including `false`. It never infers a missing old choice from public state. The Compact runtime calculation is checked against the generated retest circuit for both outcomes. Its patch input is the replayed public patch commitment: the backup does not independently retain the patch the user intended, so a match cannot establish that intention. Missing/malformed evidence or report mismatch cannot produce a match. Saved notes and choice stay outside the worker payload.

This is a comparison with source-replayed public report fields, not proof that a backup is authentic or that the transaction was authored with the original user's complete intended arguments. Indexer/RPC trust and deployed-code authentication limits remain. Intended retest patch, report preimages, actor authority and complete transaction arguments are not covered by this comparison. No recovery journal field is rewritten, no attempt is marked terminal, and no retry or wallet action is enabled. Payout authorization still does not establish a token transfer.

...
Deployment uses the shared role-style wait: fifteen minutes for SDK preparation/checkpoint, then ten minutes for confirmation after the saved identifier. Both monotonic and wall-clock deadlines are rechecked at checkpoint and result boundaries. Timely SDK success must match the saved identifier before installing the deployed API; it clears the unconfirmed marker and permits the existing report workflow. A late success cannot clear uncertainty. The browser copy retains the pre-broadcast checkpoint; the user should download an updated file after finality. The copy password is cleared from the form after initial confirmation and from the writer when the operation ends.

...
New evidence fetches have a 20-second deadline and a streamed 16 MiB response cap. The history scan retains its own limits. The UI terminates the worker on cancellation, identity change, unmount or a 90-second overall deadline, including during synchronous WASM work. The initial transaction observer now also uses the shared streamed JSON reader, capped at 1 MiB per response; public contract-state lookup and replay allow 16 MiB. The reader rejects invalid UTF-8 and cancels oversized or aborted bodies before parsing. These bounds apply to decoded response bytes, not whole-process memory or synchronous JSON/WASM execution time. Results are ephemeral and do not update journal entries or authorize another transaction.

...
The manual `release-verification.yml` workflow scans the built web runtime after hosting verification and independently pulls/scans the collector image selected by the monitoring Compose overlay. Both use `scripts/scan-container.sh` with all severities and no ignored findings. The September 9 local scans currently fail: one UNKNOWN finding in the web runtime and 14 occurrences in the collector. See [web evidence](evidence/web-current-runtime-scan.json) and [collector evidence](evidence/prometheus-runtime-scan.json). These are unresolved release gates; functional tests cannot override them. The workflow is configured locally and has not been executed on GitHub. The ciphertext runtime has its own scan in ordinary CI; its latest local evidence is recorded separately.

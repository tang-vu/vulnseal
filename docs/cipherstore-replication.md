...
Only HTTP `200` (existing identical copy) and `201` (new copy) count as PUT acknowledgments. Other 2xx replies remain unconfirmed: one `201` and one `202` report one of two acknowledged destinations. This checks the response protocol; an operator can still misrepresent storage and must be checked independently through digest-verified reads and operational evidence.
...
Use **Check stored copies** for the selected report to read every configured destination independently. The browser computes the address from the saved envelope locally, downloads and verifies each returned copy, and shows per-endpoint results with a completion timestamp. Checks issue only GET requests, use `cache: no-store`, and keep the existing 20-second per-request / 5 MiB byte bounds. Requests run in parallel across at most three endpoints. Neither a valid fallback nor a repair upload hides a failed destination. Changing the selected report or starting an upload clears the displayed check. A successful check means matching bytes were returned at that time; it does not prove future retention, physical independence or ledger finality.

The API exports `verifyCipherstoreCopies(urls, address, timeoutMs?)` with the same behavior. Its result includes the checked address, timestamp and ordered per-endpoint verification/error details, without returning the downloaded envelopes. Invalid configuration/address/timeout fails before issuing reads.

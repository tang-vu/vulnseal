...
| --- | --- | --- |
| Durable ownership and report recovery | Encrypted file recovery plus opt-in encrypted IndexedDB role autosave; revision checks prevent stale-tab overwrites; restored deployed roles verify authority/report bindings | Real Lace network recovery, pending-transaction reconciliation, backup lifecycle/deletion/quota management and cross-device recovery drills |

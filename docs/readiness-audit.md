...
| --- | --- | --- |
| Durable ownership and report recovery | Encrypted file recovery plus opt-in encrypted IndexedDB role autosave; catalog export and revision-checked deletion; stale writers cannot recreate deleted rows; isolated-browser file recovery drill; restored deployed roles verify authority/report bindings | Real Lace network recovery, pending-transaction reconciliation, quota management and physical cross-device recovery drills |

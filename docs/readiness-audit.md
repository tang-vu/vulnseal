...
| Production ciphertext operations | Local filesystem adapter; atomic writes/digest checks, aggregate quotas, bounded active uploads, restart-aware accounting and independent disk-probed readiness tested; one writer process per directory | Configured deployment and monitoring, per-user abuse controls, retention/deletion policy, backup/restore drills and replication |

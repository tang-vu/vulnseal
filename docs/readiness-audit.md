...
| Independent researcher/vendor operation | Recipient-key disclosure exchange gives separate-browser read access; fixed-role API sessions retain one secret, check fresh authority/subjects and serialize witnesses; generated-circuit simulator journey covers two actors through closure; browser still combines authority | Independent program invitations, role-scoped encrypted recovery and browser transaction journeys on real providers; receiving keys must not be mistaken for vendor authority |

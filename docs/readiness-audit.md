...
| Production ciphertext operations | Local filesystem adapter; atomic writes/digest checks, quotas, bounded uploads, disk-probed readiness and manifest-verified stopped-writer backup/restore tested; restored HTTP ciphertext decrypts in a synthetic drill; one writer process per directory | Configured deployment and monitoring, per-user abuse controls, retention/deletion policy, physical off-device/production-volume recovery drills and replication |

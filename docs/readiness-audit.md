...
| Independent researcher/vendor operation | Recipient-key disclosure exchange gives a separate browser read-only report access without actor secrets; transaction authority still lives in the combined experimental session | Independent program join/invitations, role-scoped authority and recovery, separate-browser transaction journeys; receiving keys must not be mistaken for vendor authority |

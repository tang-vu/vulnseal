...
node cipherstore/dist/retire.js apply <stopped-store> <policy-file> <reviewed-policy-digest> <new-private-audit-file>
```

Review `selected` and `present` in the plan. Copy its `policyDigest` into the apply command only after approving that exact digest inventory. The CLI rejects a changed inventory/hash before removal. This hash binds the sorted digest set, not human authorization or the store's contents; apply recalculates which approved digests remain present. The operator must verify the intended store path separately. The same policy applies to every listed digest; use an independently approved narrower policy file when only a subset is authorized for removal, and keep the full retirement guard configured on services.

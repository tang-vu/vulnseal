...
`release-verifier-keys.json` captures the eight public reference `.verifier` files retained in `contract/src/managed/vulnseal/keys` on September 10. Each is 2,119 bytes and independently matched the SDK-exposed key of the captured historical deployment. This is not fresh key generation or authenticated source provenance. Unit/browser tests use this fixture so ordinary skip-ZK CI does not require untracked proving material; the browser's real implementation downloads reference keys from its release directory. Tests intercept those downloads and include a deliberately altered reference key.

...
export async function demoTransitionWait<T>(prepare: () => Promise<unknown>, submit: () => Promise<T>, timeoutMessage?: string): Promise<T> {
  let closed = false;
  ...
  const error = new Error(timeoutMessage ?? "Stopped waiting for this transition. Its outcome is unknown and it may still finalize. Export an encrypted backup through Private recovery. Further transactions remain blocked; do not resubmit this attempt.");
  const assertActive = () => {

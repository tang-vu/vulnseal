...
const mocks = vi.hoisted(() => ({ connect: vi.fn(), deploy: vi.fn(), write: vi.fn() }));
...
beforeEach(() => {
  mocks.connect.mockReset().mockResolvedValue({}); mocks.deploy.mockReset(); mocks.write.mockReset().mockImplementation(async (...args) => row(...args as Parameters<typeof row>));
});

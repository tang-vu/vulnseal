...
export type PublicRepositoryReference = { fullName: string; url: string; archived: boolean };
...
export async function fetchPublicRepository(input: string, signal: AbortSignal): Promise<PublicRepositoryReference> {
  ...
  const request = async () => {
    ...
    const response = await fetch(`https://api.github.com/repos/${coordinates}`, {
    ...
    const buffer = new Uint8Array(256 * 1024); let length = 0;
    ...
    try {
      while (true) {
        ...
        controller.signal.throwIfAborted();
        if (done) break;
        if (value.length > buffer.length - length) throw new Error("GitHub metadata exceeds 256 KiB");
      ...
      return { fullName: repo.full_name, url: `https://github.com/${repo.full_name}`, archived: repo.archived };

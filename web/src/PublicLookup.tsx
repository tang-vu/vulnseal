...
export function PublicLookup() {
  ...
  const [operation, setOperation] = useState<"lookup" | "receipt">();
  const working = operation !== undefined;
  const [shareLink, setShareLink] = useState("");
  const controller = useRef<AbortController | undefined>(undefined);
  useEffect(() => () => controller.current?.abort(), []);
  const invalidate = () => { controller.current?.abort(); setOperation(undefined); setResult(undefined); setError(""); setShareLink(""); };
  const load = async (event: FormEvent) => {
    event.preventDefault(); invalidate();
    const pending = new AbortController(); controller.current = pending; setOperation("lookup");
    try {
    ...
    } finally { if (controller.current === pending) setOperation(undefined); }
  };
  ...
  return <section className="page narrow-page public-lookup">
    ...
    <form className="form-panel" onSubmit={(event) => void load(event)}>
      ...
      <label>Import public receipt<input type="file" accept=".json,application/json" onChange={async (event) => {
        ...
        invalidate();
        const pending = new AbortController(); controller.current = pending; setOperation("receipt");
        try {
      ...
      <button className="primary-button" disabled={working}>{operation === "lookup" ? "Checking public state…" : "Load public state"}</button>
      {working && <button type="button" className="secondary-button" onClick={invalidate}>{operation === "receipt" ? "Cancel receipt import" : "Cancel public lookup"}</button>}
    </form>

...
export const recoveryFixture = async (draft = recoveryDraft) => {
  const programId = "12".repeat(32);
  const salt = "23".repeat(32);
  const sealed = await sealReport(draft, programId);
  ...
  const snapshot: RecoverySnapshot = {
    ...
    vendorSecret: "34".repeat(32), researcherSecret: "45".repeat(32), draft,

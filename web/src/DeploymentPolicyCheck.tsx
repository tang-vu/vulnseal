...
export function DeploymentPolicyCheck({ network, transactionId, saved }: { network: string; transactionId: string; saved: SavedDeploymentInputs }) {
  ...
  return <div><button type="button" className="secondary-button" disabled={working} onClick={() => void check()}>Compare saved deployment policy</button>
    ...
    {result && <div role="status"><p>{result.mismatches.length ? `Saved deployment policy differs: ${result.mismatches.join(", ")}.` : "All seven saved deployment policy fields match the historical state reported by the indexer."}</p><p className="public-value">Deployment address: {result.address} · block {result.blockHeight} · checked {result.checkedAt}</p><p>This comparison trusts indexer state and RPC finality. It does not authenticate deployed code, constructor arguments or vendor authority, save an address, or authorize retry.</p></div>}
  </div>;
}

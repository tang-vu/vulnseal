...
export function ReportEffectCheck({ network, transactionId, contractAddress, programId, reportId, circuit, savedNotes, savedEnvelope }: Omit<ReportCheckInput, "indexerUrl" | "rpcUrl" | "websocketUrl"> & { network: string; savedNotes?: ReportNotes | null | undefined; savedEnvelope?: string | undefined }) {
  ...
  return <details><summary>Reconcile this report operation</summary>
    ...
    {result && (circuit === "submitReport" ? <SavedCiphertextCheck envelope={savedEnvelope} digest={result.publicValues?.ciphertextDigest} /> : <SavedDecisionCheck circuit={circuit} reportId={reportId} notes={savedNotes} values={result.publicValues} />)}
  </details>;
}

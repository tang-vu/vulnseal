  ...
  return <div className="app-shell role-workspace">
    ...
    <main id="main-content" className="page narrow-page"><h1 ref={heading} tabIndex={-1}>{vault ? `${vault.role === "vendor" ? "Vendor" : "Researcher"} workspace` : "Work with your own authority"}</h1>
      ...
      {vault && <section className="form-panel"><h2>Submission journal</h2>
        ...
        {vault.submissionAttempts?.length ? <JournalEntries entries={vault.submissionAttempts} label="Search submission journal">{(entry) => <li className="public-value" key={entry.transactionId}>{entry.transactionId} · recorded {entry.recordedAt} · outcome requires reconciliation<SubmissionIntentView entry={entry} includePrivateNotes /><TransactionCheck network={vault.network} transactionId={entry.transactionId} contractAddress={vault.contractAddress} circuit={entry.intent?.circuit} />{vault.contractAddress && entry.intent?.reportId && <ReportEffectCheck network={vault.network} transactionId={entry.transactionId} contractAddress={vault.contractAddress} programId={vault.programId} reportId={entry.intent.reportId} circuit={entry.intent.circuit} savedNotes={entry.notes} savedRetestPassed={entry.retestPassed} savedRetestPatch={entry.retestPatchCommitment} savedEnvelope={vault.reports.find((report) => report.reportId === entry.intent?.reportId)?.envelope} />}</li>}</JournalEntries> : <p>No recorded submission attempts.</p>}
      </section>}

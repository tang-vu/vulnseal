...
export function GitHubScopeImport({ onApply }: { onApply: (url: string) => void }) {
  ...
  const lookup = async () => {
    ...
    try { const result = await fetchPublicRepository(url, controller.signal, pinCommit); if (current.current === controller) setReference(result); }
  ...
  return <section aria-label="Import public GitHub scope">
    ...
    <label><input type="checkbox" checked={pinCommit} onChange={event => { stop(); setReference(undefined); setError(""); setPinCommit(event.target.checked); }} />Pin scope to the current HEAD commit</label>
    ...
    {reference && <div role="status"><p>{reference.fullName} — {reference.archived ? "Archived repository" : "Public repository"}</p><p className="public-value">{reference.url}</p>{reference.commitSha && <p className="public-value">Pinned commit: {reference.commitSha}. This scope does not follow later repository changes.</p>}<button type="button" onClick={() => { onApply(reference.url); setReference(undefined); }}>Use repository as primary scope</button></div>}
  </section>;
}

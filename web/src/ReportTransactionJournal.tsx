...
export function ReportTransactionJournal({ attempts, network, contractAddress }: { attempts: readonly ReportAttempt[]; network: string; contractAddress: string }) {
  ...
  return <section className="form-panel" aria-label="Report transaction journal">
    ...
    {attempt.transactionId ? <><p className="public-value">Transaction: {attempt.transactionId}</p><TransactionCheck key={attempt.transactionId} network={network} transactionId={attempt.transactionId} contractAddress={contractAddress} circuit={attempt.circuit} /></> : <p>No identifier was saved for this attempt. This does not prove that no transaction was sent.</p>}

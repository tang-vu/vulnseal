...
export type ReplayedPublicValues = { decisionDigest: string; severity: string; rewardTier: string; ciphertextDigest?: string; patchCommitment?: string; retestCommitment?: string; retestPassed?: boolean };
...
export async function reconcileReport(input: ReportCheckInput): Promise<ReportCheckResult> {
  ...
  return { checkedAt: new Date().toISOString(), reportId: input.reportId, before: change.before ? contractStatusName(change.before.status) : "Absent", after: change.after ? contractStatusName(change.after.status) : "Removed", blockHeight: observation.blockHeight, previousBlockHeight: history.previous.blockHeight, actionsRead: history.actionsRead, ...(change.after ? { publicValues: { retestCommitment: bytesToHex(change.after.retestCommitment), retestPassed: change.after.retestPassed, patchCommitment: bytesToHex(change.after.patchCommitment), ciphertextDigest: bytesToHex(change.after.ciphertextDigest), decisionDigest: bytesToHex(change.after.decisionDigest), severity: change.after.severity.toString(), rewardTier: change.after.rewardTier.toString() } } : {}) };
}

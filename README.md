...
Open `http://127.0.0.1:5173`. Guided Local mode performs real canonicalization, SHA-256 commitment preparation, AES-GCM encryption, and ciphertext storage, while clearly labeling workflow transitions as a guided product demonstration. For actual Midnight transactions, connect a compatible Lace wallet, create a network program, and then submit a new report. Connecting a wallet does not convert a guided report into a network report. Network program creation requires a deployment backup password: encrypted browser checkpoints are saved before SDK work and before broadcast, then updated with the confirmed contract address. Keep an independent file backup. The [combined recovery guide](docs/combined-recovery.md) explains saved copies, autosave, restoration and interrupted attempts.
...
Compilation now stages output under `.compact/compile-*`. Failed compilation or missing required artifacts leaves the installed directory in place. A successful full compile requires nonempty prover/verifier/binary-ZKIR files for each proving circuit, then archives the previous directory beside the staged output before installing the new one. Keep the printed archive path until the new release is verified. This checks file completeness, not key semantics. `.compact/compile.lock` excludes cooperating compiler runs; an interrupted process may leave a lock that requires inspection before manual removal. Installation uses two directory renames with rollback on a reported second-rename error; a machine/process crash between renames still requires manual recovery from the printed staging directory. Do not build or serve artifacts concurrently with installation. Archives and failed staging directories are retained for inspection, not automatically pruned.

Build copying replaces generated `dist` directories instead of merging them, so removed circuits and absent source keys cannot leave stale key files in a new build. Each replacement uses a prepared copy and attempts rollback on installation error. Publish only a complete checked release; directory replacement is not atomic across all build outputs.
...
In the role workspace, preparing a report encrypts it locally first. Save its encrypted role backup, then use **Upload saved ciphertext** in **Reports**. A lost storage response can be retried from the same restored backup without changing the report ID or encryption material. **Submit prepared report** uploads those same saved bytes before starting the contract call; storage failure stops that call. See [the recovery design](docs/adr/0021-prepared-ciphertext-recovery.md).

The combined demo also retains a pending encrypted report after an upload error. Use **Private recovery** to save it before closing; restoring that file lets a storage-only retry reuse the original bytes. Once a network submission attempt has started, retry stays blocked because the outcome needs investigation. This manual backup is weaker than the role workspace's durable submission journal; an older file cannot prove that a later submission never happened. See [pending preparation recovery](docs/adr/0022-pending-demo-preparation.md).

Optional `VITE_CIPHERSTORE_REPLICAS` adds up to two ciphertext destinations. Uploads require acknowledgment from every configured store; reads fall back after an error or digest mismatch. The UI lists these destinations, and existing backups retain the original envelope for explicit reupload. See [replication configuration and its limits](docs/cipherstore-replication.md).

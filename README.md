...
The role workspace supports password-encrypted browser autosave for identity, prepared/received reports and the current incomplete report draft, with fresh-tab unlock and revision checks against conflicting tab writes. Real role submissions require autosave so the transaction identifier can be persisted before calling the wallet; the [submission journal](docs/adr/0012-submission-journal.md) records attempts without claiming success or finality. Keep a downloaded backup too: clearing browser data removes local copies. Transition notes and receiving keys are outside this autosave. Draft edits are saved after a short pause in typing; wait for the saved confirmation. Select **Restore backups without connecting Lace** to recover local drafts from files or browser copies when the wallet is unavailable. Offline recovery does not establish current contract authority; connect and verify the program before transactions. See [the storage design](docs/adr/0011-encrypted-browser-autosave.md).

The role workspace requests a browser leave warning while its identity/reports/draft are unsaved, an operation is running, transition notes remain, or receiving keys are loaded. Receiving keys keep the warning active even after a separate key export because the workspace cannot confirm that file was retained. This warning is best effort: browsers can suppress it, and crashes or mobile app termination may bypass it. The warning itself does not save data; wait for draft persistence and retain notes and the separate encrypted receiving-key backup before leaving.

...
export function inspectTransactionContent(raw: string, identifier: string, expectedHash: string) {
  ...
  const tx = Transaction.deserialize("signature", "proof", "binding", Buffer.from(encoded, "hex"));
  ...
  for (const [segment, intent] of tx.intents ?? []) {
    for (const action of intent.actions) {
      ...
      calls.push({ segment, address: action.address, entryPoint: typeof action.entryPoint === "string" ? action.entryPoint : { hex: Buffer.from(action.entryPoint).toString("hex") }, guaranteedOperations: action.guaranteedTranscript?.program.length ?? 0, fallibleOperations: action.fallibleTranscript?.program.length ?? 0 });
    }
  }
